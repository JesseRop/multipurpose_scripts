...
def main(read_bam, output_bam, namefile, labl, tag_name):
    ...
    for r in reader:
        try:
            ...
            d2 = r.to_dict()
            if r.get_tag('CB') in d:
                d2['tags'].append('XS:Z:' + str(d[r.get_tag('CB')][0]))
                d2['tags'].append(tag_name + str(d[r.get_tag('CB')][1]))
            x = pysam.AlignedSegment.from_dict(d2, r.header)
            f2.write(x)

        except KeyError:
            # ignore the CB tag not present error and continue to the next line
            continue
